...
tap.test('GET `/failz` route', (t) => {
  const fastify = buildFastify()

  t.plan(3)
  t.teardown(() => fastify.close())
  fastify.inject({ method: 'GET', url: '/failz' }, (err, response) => {
...
tap.test('GET `/healthz` route', (t) => {
  const fastify = buildFastify()
  const packageJson = require('./../package.json')

  t.plan(5)
  t.teardown(() => fastify.close())
  fastify.inject({ method: 'GET', url: '/healthz' }, (err, response) => {
